Shader "Simple CRT"
{
    Properties
    {
        ...
        _MainCRTDistortion ("CRT Distortion", Vector) = (0.1, 0.15, 0, 0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float2 crtDistortion = _MainCRTDistortion.xy; // Використовуємо значення з матеріалу
                float2 crtCenter = float2(0.5, 0.5);
                float2 crtUv = (uv - crtCenter) * (1.0 - dot(uv - crtCenter, uv - crtCenter) * crtDistortion);
                crtUv += crtCenter;
                uv = crtUv;
                ...
                float red = tex2D(_MainTex, float2(uv.x - _ChromaticAberrationStrength * _ChromaticAberrationOnOff,
                        uv.y)).r;
                float green = tex2D(_MainTex, float2(uv.x, uv.y)).g;
                float blue = tex2D(_MainTex, float2(uv.x + _ChromaticAberrationStrength * _ChromaticAberrationOnOff,
                        uv.y)).b;
                ...
                /////Scanline
                float scanlineUvY = uv.y + _Time.x; // Використовуємо вже змінений uv
                float scanline = sin(scanlineUvY * _ScanlineCount) * 0.04; // Використовуємо змінну _ScanlineCount
